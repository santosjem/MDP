...
int main(int argc, char* argv[])
{
  ...
  unsigned int state0 = (unsigned int) strtol(argv[2], NULL, 10);
  ...
  { 
    ...
    int numAvailableActions = p_mdp->numAvailableActions[state];
    printf("[%d] ", numAvailableActions);
    for(unsigned int action = 0; action < numAvailableActions; action++) {
      printf("%d ", p_mdp->actions[state][action]);
    }


    printf("%lf \n", p_mdp->transitionProb[state][state0][action]);
